...
resource "aws_s3_bucket" "redirect" {
  bucket        = substr(format("%s-%s", "redirect-mys2msh-${var.env_name}", replace(random_uuid.some_uuid.result, "-", "")), 0, 32)
  force_destroy = true
}
...
resource "aws_cloudfront_function" "redirect_2_www_function" {
  name    = "redirect-mys2msh-function"
...
resource "aws_cloudfront_origin_access_control" "s3_only_access_control" {
  name                              = "orig-s3-for-redirect-bucket-mys2msh"
...
resource "aws_cloudfront_distribution" "redirect_dist" {
  ...
  default_cache_behavior {
    ...
    target_origin_id       = "redirect-mys2msh"
  ...
  aliases             = [var.route53_domain,"www.${var.route53_domain}"]
  wait_for_deployment = false
  price_class         = "PriceClass_100"
  enabled             = true
  is_ipv6_enabled     = true
  http_version        = "http2"
  comment             = "redirect-mys2msh"
