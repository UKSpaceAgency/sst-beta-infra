module "e2e-backend" {
  ...
  custom_command = [
    "sh",
    "-c",
    "poetry run alembic upgrade head && poetry run python -m _scripts.fe_mock && poetry run uvicorn --host=0.0.0.0 --port=8080 app.main:app"
  ]
  ...
  secret_env_vars = [
    {
      "name" : "DATABASE_URL",
      "valueFrom" : "${data.aws_secretsmanager_secret.by-name.arn}:E2EdatabaseUrlE2E::"
    },
    {
      "name" : "AUTH0_ISSUER",
